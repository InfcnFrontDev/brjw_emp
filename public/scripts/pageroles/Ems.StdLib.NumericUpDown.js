...
Ems.StdLib.NumericUpDown = fabric.util.createClass({
    ...
    init: function (options) {
        ...
        io.value = this.option.Value;
        if (this.option.MultiLine) {  //多行；50
            io.rows = "50";
        }

        $('#' + this.inputboxid).spinner({
            max: this.option.Maximum,
            min: this.option.Minimum,
            step: this.option.Increment
        });
